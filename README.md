...
Voici quelques captures d'écran montrant plusieurs prototypes de programme de ce genre.
...
![Températures maximales par département le 01/07/2018](capture_dep.png){ width=60% }
...
![Températures maximales par station météo le 01/07/2018](capture_stations.png){ width=60% }
...
![Licenciés de basket-ball en 2011 en Seine-et-Marne](capture_basket.png){ width=60% }
...
Chaque disque représente une donnée localisée à des coordonnées précises, exprimées dans le format [WGS 84](https://fr.wikipedia.org/wiki/WGS_84) déjà mentionné (une station météo ou le nombre de licenciés d'un club de basket dans les exemples ci-dessus). Cette information a été récupérée dans les fichiers CSV indiqués sous chaque exemple.
...
- une évolution des données au cours du temps si l'on dispose de plusieurs jeux de données sur une certaine période.